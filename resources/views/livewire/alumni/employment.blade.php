...
.c-pill {
    display:inline-flex; align-items:center; gap:8px;
    padding:7px 14px; border:1.5px solid #e5e7eb; border-radius:.75rem;
...
.s-head  { display:flex; align-items:center; gap:10px; padding:10px 18px; border-bottom:1px solid #f3f4f6; background:var(--brand-light); }
.s-icon  { width:32px; height:32px; border-radius:10px; display:flex; align-items:center; justify-content:center; background:var(--brand); flex-shrink:0; }
.s-label { font-size:.7rem; font-weight:700; letter-spacing:.06em; text-transform:uppercase; color:#6b7280; }
.e-msg   { font-size:.75rem; color:#ef4444; display:flex; align-items:center; gap:4px; margin-top:3px; }
.b-pill  { display:inline-flex; align-items:center; gap:5px; font-size:.75rem; font-weight:600; padding:4px 10px; border-radius:99px; border:1px solid; }
...
.hist-head {
    ...
    padding:18px 22px 16px; border-bottom:1px solid #f3f4f6; flex-shrink:0;
}
.hist-body { overflow-y:auto; padding:20px 22px; flex:1; }
.hist-foot { padding:14px 22px; border-top:1px solid #f3f4f6; flex-shrink:0; display:flex; justify-content:flex-end; }
...
.tl-card {
    ...
    border-radius:.875rem; padding:14px 16px;
}
.tl-card.tl-cur {
    background:var(--brand-light); border-color:#c4b5d9;
    box-shadow:0 2px 14px rgba(122,63,145,.11);
}
.tl-meta { font-size:.7rem; color:#9ca3af; font-weight:600; letter-spacing:.04em; margin-bottom:8px; }
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
    <div>
        <h1 class="text-xl font-extrabold text-gray-900 tracking-tight">Employment Tracking</h1>
...
<div class="s-card">
    <div class="s-head">
        ...
        <div class="flex-1">
            <p class="text-sm font-bold text-gray-900">Employment Status</p>
            <p class="text-xs text-gray-500">Your current employment situation</p>
        </div>
        @if(!$editing)
        ...
        @if($editing)
            <div class="flex flex-wrap gap-3">
                <label class="r-pill">
                    ...
                    <span class="font-medium text-gray-700 text-sm">Employed</span>
                </label>
                <label class="r-pill">
                    <input wire:model.live="employment_status" type="radio" value="self_employed" class="w-4 h-4 accent-blue-600">
                    <i class="fa-solid fa-store text-blue-500 text-xs"></i>
                    <span class="font-medium text-gray-700 text-sm">Self-Employed</span>
                </label>
                <label class="r-pill">
                    <input wire:model.live="employment_status" type="radio" value="unemployed" class="w-4 h-4 accent-orange-500">
                    <i class="fa-solid fa-magnifying-glass text-orange-400 text-xs"></i>
                    <span class="font-medium text-gray-700 text-sm">Unemployed</span>
                </label>
            </div>
        ...
        @else
            ...
            @if($s)
                <span class="b-pill {{ $s[2] }} {{ $s[3] }}">
                    <i class="fa-solid {{ $s[1] }} text-[11px]"></i> {{ $s[0] }}
                </span>
            @else
...
<div class="s-card">
    <div class="s-head">
        ...
        <div class="flex-1">
            <p class="text-sm font-bold text-gray-900">Employment Details</p>
            <p class="text-xs text-gray-500">Company, position, type &amp; location</p>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
                ...
                <input wire:model="company_name" type="text"
                       ...
                       class="w-full px-3 py-2 text-sm rounded-xl transition-all f-edit{{ $errors->has('company_name') ? ' err' : '' }}">
            ...
            <div>
                ...
                <input wire:model="job_title" type="text" placeholder="E.G. SOFTWARE DEVELOPER"
                       oninput="this.value=this.value.toUpperCase()"
                       class="w-full px-3 py-2 text-sm rounded-xl transition-all f-edit{{ $errors->has('job_title') ? ' err' : '' }}">
            ...
            <div class="flex flex-wrap gap-2">
                @foreach([
                    ...
                    <label class="r-pill">
                        ...
                        <i class="fa-solid {{ $ico }} text-violet-400 text-xs"></i>
                        <span class="font-medium text-gray-700 text-sm">{{ $lbl }}</span>
                    </label>
                @endforeach
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
                ...
                <div class="flex gap-2">
                    <label class="r-pill">
                        ...
                        <span class="font-medium text-gray-700 text-sm">Local</span>
                    </label>
                    <label class="r-pill">
                        ...
                        <span class="font-medium text-gray-700 text-sm">Abroad</span>
                    </label>
                </div>
            ...
            <div>
                ...
                <input wire:model="date_hired" type="date" max="{{ date('Y-m-d') }}"
                       class="w-full px-3 py-2 text-sm rounded-xl transition-all f-edit{{ $errors->has('date_hired') ? ' err' : '' }}">
...
<div class="s-card">
    <div class="s-head">
        ...
        <div class="flex-1">
            <p class="text-sm font-bold text-gray-900">Career Path</p>
        ...
        <div class="flex flex-wrap gap-2">
            @foreach([
                ...
                <label class="c-pill">
                    ...
                    <i class="fa-solid {{ $ico }} text-cyan-500 text-xs"></i>
...
<div class="s-card">
    <div class="s-head">
        <div class="s-icon" style="background:#7c3aed;"><i class="fa-solid fa-graduation-cap text-white text-xs"></i></div>
        <div class="flex-1">
            <p class="text-sm font-bold text-gray-900">Course Relevance</p>
            <p class="text-xs text-gray-500">Is your job related to your college course?</p>
        </div>
    </div>
    <div class="p-4">
        ...
        <div class="flex flex-wrap gap-2">
            <label class="r-pill">
                ...
                <span class="font-medium text-gray-700 text-sm">Yes — Related</span>
            </label>
            <label class="r-pill">
                ...
                <span class="font-medium text-gray-700 text-sm">No — Not Related</span>
            </label>
            <label class="r-pill">
                <input wire:model="course_relevance" type="radio" value="partially" class="w-4 h-4 accent-amber-500">
                <i class="fa-solid fa-circle-half-stroke text-amber-400 text-xs"></i>
                <span class="font-medium text-gray-700 text-sm">Partially</span>
            </label>
        </div>
...
<div class="s-card">
    <div class="s-head">
        ...
        <div class="flex-1">
            <p class="text-sm font-bold text-gray-900">Unemployment Status</p>
            <p class="text-xs text-gray-500">Are you actively looking for work?</p>
        </div>
    </div>
    <div class="p-4">
        ...
        <div class="flex flex-wrap gap-2">
            <label class="r-pill">
                ...
                <i class="fa-solid fa-person-walking text-violet-500 text-xs"></i>
                <span class="font-medium text-gray-700 text-sm">Actively Seeking Employment</span>
            </label>
            <label class="r-pill">
                ...
                <span class="font-medium text-gray-700 text-sm">Not Currently Looking</span>
            </label>
        </div>
...
<div class="s-card">
    <div class="s-head">
        ...
        <div class="flex-1">
            <p class="text-sm font-bold text-gray-900">Further Education</p>
            <p class="text-xs text-gray-500">Are you pursuing a graduate degree?</p>
        </div>
        @if(!$editing)
            <span class="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full bg-gray-100 text-gray-500">
                <i class="fa-solid fa-eye text-xs"></i> View Only
        ...
        @if($editing)
            <div class="flex flex-wrap gap-2">
                <label class="r-pill">
                    ...
                    <span class="font-medium text-gray-700 text-sm">None</span>
                </label>
                <label class="r-pill">
                    <input wire:model="education_status" type="radio" value="pursuing_masteral" class="w-4 h-4 accent-blue-600">
                    <i class="fa-solid fa-scroll text-blue-500 text-xs"></i>
                    <span class="font-medium text-gray-700 text-sm">Pursuing Masteral</span>
                </label>
                <label class="r-pill">
                    <input wire:model="education_status" type="radio" value="pursuing_doctorate" class="w-4 h-4 accent-violet-600">
                    <i class="fa-solid fa-hat-wizard text-violet-500 text-xs"></i>
                    <span class="font-medium text-gray-700 text-sm">Pursuing Doctorate</span>
                </label>
            </div>
        ...
        @else
            ...
            @if($e)
                <span class="b-pill {{ $e[2] }} {{ $e[3] }}">
                    <i class="fa-solid {{ $e[1] }} text-[11px]"></i> {{ $e[0] }}
                </span>
            @else
                <span class="text-gray-400 text-sm">—</span>
            @endif
...
<div class="s-card">
    <div class="s-head">
        ...
        <div class="flex-1">
            <p class="text-sm font-bold text-gray-900">Employment &amp; Career Summary</p>
            <p class="text-xs text-gray-500">Your current submitted details</p>
        </div>
        <span class="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full bg-gray-100 text-gray-500">
            <i class="fa-solid fa-eye text-xs"></i> View Only
        </span>
    ...
    <div class="p-4 space-y-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
                ...
                <div class="px-3 py-2 rounded-xl f-view text-sm font-semibold uppercase">{{ $company_name ?: '—' }}</div>
            </div>
            <div>
                <p class="s-label mb-1">Job Title / Position</p>
                <div class="px-3 py-2 rounded-xl f-view text-sm font-semibold uppercase">{{ $job_title ?: '—' }}</div>
            </div>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div>
                ...
                <div class="px-3 py-2 rounded-xl f-view text-sm font-semibold">{{ $tM[$employment_type] ?? '—' }}</div>
            </div>
            <div>
                <p class="s-label mb-1">Work Location</p>
                <div class="px-3 py-2 rounded-xl f-view text-sm font-semibold flex items-center gap-1.5">
            ...
            <div class="flex flex-wrap gap-2">
                @foreach($career_path as $cp)
                    <span class="b-pill text-cyan-700 bg-cyan-50 border-cyan-200">
                        <i class="fa-solid fa-check text-[10px]"></i> {{ $cpL[$cp] ?? $cp }}
                    </span>
                @endforeach
            </div>
        </div>
...
<div class="s-card">
    <div class="s-head">
        <div class="s-icon" style="background:#d97706;"><i class="fa-solid fa-magnifying-glass text-white text-xs"></i></div>
        <div class="flex-1">
            <p class="text-sm font-bold text-gray-900">Unemployment Details</p>
        </div>
        <span class="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full bg-gray-100 text-gray-500">
            <i class="fa-solid fa-eye text-xs"></i> View Only
        </span>
    ...
    <div class="hist-modal">
        ...
        <div class="hist-head">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="font-extrabold text-gray-900 text-base leading-tight">Employment History</p>
        ...
        <div class="hist-body">
            @if(empty($history))
                <div class="hist-empty">
                    ...
                    <p class="font-semibold text-gray-400 text-sm">No employment records found.</p>
                </div>
            @else
                <div class="tl-wrap">
                    {{-- Vertical line --}}
                        ...
                        <div class="tl-card {{ $isCur ? 'tl-cur' : '' }}">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-3">
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    <span class="b-pill {{ $sBadge[0] }} {{ $sBadge[1] }}">
                                        <i class="fa-solid {{ $sBadge[2] }} text-[10px]"></i>
                                        {{ $sLabel }}
                                    </span>
                                ...
                                {{-- Entry number --}}
                                <span class="text-xs font-bold text-gray-300 flex-shrink-0">#{{ $entryNum }}</span>
                            </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 mb-3
                                ...
                                <div>
                                    ...
                                    <p class="text-sm font-extrabold text-gray-800 uppercase tracking-wide">
                                        {{ $entry['company_name'] }}
                                    </p>
                                </div>
                                @endif
                            ...
                            <div class="flex flex-wrap gap-1.5">
                                ...
                                @if($isWork && $entry['work_location'])
                                    ...
                                    <span class="b-pill {{ $lc }}">
                                        <i class="fa-solid {{ $li }} text-[10px]"></i>
                                        {{ $entry['work_location'] }}
                                    </span>
                                @endif
                            ...
                            <div class="flex flex-wrap gap-1.5 mt-2.5 pt-2.5 border-t border-gray-200/60">
                                ...
                                @foreach($entry['career_path_labels'] as $cp)
                                    <span class="b-pill text-cyan-700 bg-cyan-50 border-cyan-200">
                                        <i class="fa-solid fa-check text-[9px]"></i> {{ $cp }}
                                    </span>
                                @endforeach
                            </div>
                            @endif
        ...
        <div class="hist-foot">
            <button wire:click="closeHistory"
                    class="px-6 py-2.5 rounded-xl font-bold text-sm border-2 border-gray-300
