...
/* Single lightweight fade — no per-card stagger */
@keyframes ybFadeIn {
    from { opacity: 0; }
    to   { opacity: 1; }
}
.yb-grid-wrap {
    animation: ybFadeIn .18s ease-out both;
}
</style>
...
<div class="flex flex-col flex-1 px-3 sm:px-5 lg:px-6 pt-5 max-w-screen-2xl mx-auto w-full overflow-hidden">
    ...
    <div class="rounded-2xl border border-[#E8E0F0] shadow-sm overflow-hidden mb-4 flex-shrink-0"
        ...
        <div class="px-5 py-3 flex flex-wrap gap-2 items-center">
            ...
            {{-- Search — single wire:model.live.debounce, no Alpine entangle needed --}}
        ...
        <div wire:loading
            ...
            <div class="h-0.5 rounded-full overflow-hidden" style="background:#f0e6f8;">
                <div class="h-full rounded-full animate-pulse"
                     style="background:#7A3F91; width:65%;"></div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-[#E8E0F0] shadow-sm flex flex-col flex-1 min-h-0 overflow-hidden">
        ...
        <div class="relative flex-1 min-h-0" x-data="{ showTop: false }">
            <div id="yb-scroll"
                ...
                {{-- Cards — wrapped in single fade div, NO per-card stagger --}}
                @if($this->alumniRecords->count() > 0)
                    <div class="yb-grid-wrap grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3 sm:gap-4">
                        @foreach($this->alumniRecords as $alumni)
                        <div wire:key="alumni-{{ $alumni->id }}"
                             class="bg-white rounded-2xl overflow-hidden border border-[#E8E0F0]
                                    shadow-sm flex flex-col items-center
                                    hover:border-[#d4aaeb] hover:shadow-md transition-all duration-150">
