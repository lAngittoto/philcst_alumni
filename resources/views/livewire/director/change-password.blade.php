{{-- resources/views/livewire/director/change-password.blade.php --}}
...
new #[Layout('app')] class extends Component {
    ...
    public function mount(): void
    {
        ...
        if ($director->password_changed_at !== null) {
            session()->forget('director_requires_password_change');
    ...
    public function changePassword(): void
    {
        ...
        if (!$director) {
            $this->errorMessage = 'Director record not found.';
            return;
        }

        try {
            DB::transaction(function () use ($user, $director) {
                ...
                // Mark password as changed + activate director
                DB::table('director')
            ...
            Log::info("Director password changed: user #{$user->id} (director #{$director->id})");

            $this->redirect(route('director.dashboard'));

        } catch (\Exception $e) {
            Log::error("Director changePassword error: " . $e->getMessage());
...
>
    {{-- Overlay --}}
    <div class="absolute inset-0 bg-black/55 backdrop-blur-sm z-0"></div>
    ...
    <div class="relative z-10 w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div class="px-6 pt-6 pb-5 border-b border-gray-200">
            <div class="flex items-center gap-3">
                <div class="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0" style="background:#F5EEF9;">
                    <i class="fa-solid fa-lock text-base" style="color:#7A3F91;"></i>
                </div>
                <div>
                    <h1 class="text-lg font-bold text-gray-900 leading-tight">Set Your Password</h1>
                    <p class="text-sm text-gray-500 mt-0.5">Secure your director account before continuing</p>
                </div>
            </div>
        </div>
        ...
        <div class="p-6 space-y-5">
            ...
            @php
                $dirRecord = DB::table('director')->where('user_id', auth()->id())->first();
                $displayName = $dirRecord
                    ...
                    : auth()->user()?->name;
                $loginUsername = explode('@', auth()->user()?->email ?? '')[0] ?? '—';
            @endphp
            <div class="rounded-xl p-4 flex items-center gap-3" style="background:#F5EEF9;border:1px solid #e9d5f3;">
                <div class="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0" style="background:#EDE9F6;">
                    <i class="fa-solid fa-user-tie text-sm" style="color:#7A3F91;"></i>
                </div>
                <div>
                    <p class="text-sm font-semibold text-gray-900">{{ $displayName }}</p>
                    <p class="text-xs font-medium mt-0.5" style="color:#7A3F91;">Director · @<span class="font-mono">{{ $loginUsername }}</span></p>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    <input wire:model.live="password"
                           ...
                           class="w-full px-4 py-3 text-base border-2 border-gray-300 rounded-xl focus:outline-none focus:ring-2 transition-all pr-12 bg-white text-gray-900"
                           style="focus:border-color:#7A3F91;">
            ...
            <button wire:click="changePassword"
                    ...
                    class="w-full py-3 rounded-xl font-semibold text-base text-white shadow-md active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2 mt-2"
        ...
        <div class="px-6 py-4 text-center bg-gray-50 border-t border-gray-200">
            <p class="text-xs text-gray-500 font-medium">&copy; {{ date('Y') }} Philippine College of Science and Technology</p>
        </div>
    </div>
</div>