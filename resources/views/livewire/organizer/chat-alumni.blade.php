...
new class extends Component {
    ...
    {
        $staffRoomRow = DB::table('chat_rooms')
        ...
        $this->availableCourses = $batchRooms
            ->pluck('course_code')->filter()
            ->map(fn ($c) => strtoupper($c))->unique()->sort()->values()->toArray();

        $this->availableBatches = $batchRooms
            ->pluck('batch')->filter()
        ...
        $combined = collect();
        if ($this->roomTypeFilter !== 'batch' && $staffItem) {
            $combined->push($staffItem);
        }
        if ($this->roomTypeFilter !== 'staff') {
    ...
    public function loadMessages(): void
    {
        ...
        $this->messages = collect($rows)->map(function ($m) use ($aMap, $oMap, $dMap, $rxns, $pins, $rplyMap, $self) {
            ...
            $s = $isDir
                ? $dMap->get($sid)
                : ($isCoord ? $oMap->get($sid) : $aMap->get($sid));

            if (! $s && $isCoord && $sid === $self->coordinatorId) {
                $sName = $self->coordinatorName;
            } else {
            ...
            $myRxn = $msgRxns->first(
                fn ($r) => $r->reactor_type === 'organizer' && (int) $r->reactor_id === $self->coordinatorId
            );
            ...
            if ($m->reply_to_id && $rplyMap->has((int) $m->reply_to_id)) {
                ...
                $rs = $r->sender_type === 'director'
                    ? $dMap->get((int) $r->sender_id)
                    : ($r->sender_type === 'organizer'
    ...
    public function loadStaffMembers(): void
    {
        ...
        $this->staffDirectors = $dirQuery->orderBy('first_name')
            ->get(['id', 'first_name', 'last_name', 'profile_photo', 'last_seen_at'])
            ->map(fn ($d) => [
                'id'        => $d->id,
    ...
    public function loadPins(): void
    {
        ...
        $this->pinnedMessages = collect($rows)->map(function ($p) use ($aMap, $oMap, $dMap) {
            $s = $p->sender_type === 'director'
                ? $dMap->get((int) $p->sender_id)
                : ($p->sender_type === 'organizer'
...
<div class="flex rounded-2xl border border-[#ddd3e8] bg-white shadow-sm overflow-hidden"
    ...
    <div class="w-80 flex-shrink-0 flex flex-col border-r border-[#ddd3e8] bg-white">
        ...
        {{-- Sidebar header — SOLID PURPLE, no gradient --}}
        <div class="px-4 py-3.5 border-b border-[#5c2778] flex-shrink-0"
        ...
        <div class="px-3 pt-3 pb-2 border-b border-[#ddd3e8] flex-shrink-0 space-y-2.5 bg-[#fafafa]">
            ...
            <div>
                ...
                <div class="grid grid-cols-3 gap-1.5">
                    @foreach(['' => 'All', 'staff' => 'Staff Chat', 'batch' => 'Batch GC'] as $val => $label)
                    <button wire:click="$set('roomTypeFilter', '{{ $val }}')"
                            class="flex items-center justify-center gap-1 py-1.5 px-2 rounded-lg border
            ...
            {{-- Batch filter --}}
            @if($roomTypeFilter !== 'staff' && count($availableBatches) > 0)
            <div>
                <label class="text-[10px] font-semibold text-[#999999] uppercase tracking-widest block mb-1.5">
                ...
                <div class="relative">
                    <select wire:model.live="batchFilter"
                            class="w-full appearance-none text-sm font-semibold text-[#333333] bg-white
                                   border border-[#ddd3e8] rounded-lg pl-3 pr-8 py-2
                                   focus:outline-none focus:border-[#6b2490] focus:ring-2 focus:ring-[#6b2490]/20
        ...
        {{-- Chat list label — "All Chats" not "All Rooms" --}}
        <div class="px-4 pt-3 pb-1.5 flex-shrink-0 bg-white">
            <div class="flex items-center justify-between">
                <p class="text-xs font-semibold text-[#999999] uppercase tracking-widest">
                    ...
                    {{ $roomTypeFilter === 'staff' ? 'Staff Chat' : ($roomTypeFilter === 'batch' ? 'Batch Chats' : 'All Chats') }}
                </p>
        ...
        <div class="flex-1 overflow-y-auto px-2 pb-3 space-y-0.5 bg-white">
            ...
            <button wire:click="selectRoom({{ $r['id'] }})"
                    class="w-full text-left rounded-xl px-3 py-3 transition-all border
                           {{ $r['is_active']
                               ? 'border-[#c49bdb] bg-[#f2e8f9]'
                               : 'border-transparent hover:border-[#ddd3e8] hover:bg-[#fafafa]' }}">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-start justify-between gap-1">
                            <p class="text-sm font-semibold leading-tight truncate
                                      {{ $r['is_active'] ? 'text-[#6b2490]' : 'text-[#333333]' }}">
                                Staff Chat
                            </p>
                            @if($r['latest_time'])
            ...
            @if($roomTypeFilter === '' && count($rooms) > 1)
            <div class="flex items-center gap-2 px-1 py-1.5">
                <div class="flex-1 h-px bg-[#ddd3e8]"></div>
                <span class="text-[10px] font-semibold text-[#999999] uppercase tracking-widest">Batch Chats</span>
                <div class="flex-1 h-px bg-[#ddd3e8]"></div>
            </div>
            @endif
            ...
            <div class="flex flex-col items-center justify-center py-16 text-[#999999] text-center px-4">
                ...
                @if($courseFilter !== '' || $batchFilter !== '')
                <button wire:click="$set('courseFilter', ''); $set('batchFilter', '');"
                        class="mt-3 text-xs font-semibold text-[#6b2490] hover:underline">
                    Clear filters
                </button>
                @endif
            </div>
    ...
    <div class="flex flex-1 min-w-0 flex-col">
        ...
        <div class="flex items-center gap-3 px-5 py-3.5 flex-shrink-0 border-b border-[#5c2778]"
            ...
            <div class="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 bg-white/20 border border-white/30">
                <i class="fa-solid {{ $isStaffRoom ? 'fa-shield-halved' : 'fa-users' }} text-white text-sm"></i>
            </div>

            <div class="flex-1 min-w-0">
                <p class="text-white font-semibold text-sm leading-tight truncate uppercase tracking-wide">
                    {{ $isStaffRoom ? 'Staff Chat' : ($room['name'] ?? 'Group Chat') }}
                </p>
            ...
            <div class="flex items-center gap-1.5 flex-shrink-0">
                <button wire:click="togglePins"
                        ...
                        style="{{ $showPins
                            ? 'background:rgba(255,255,255,.28);color:#fff;border-color:rgba(255,255,255,.40);'
                            : 'background:rgba(255,255,255,.12);color:rgba(255,255,255,.75);border-color:rgba(255,255,255,.20);' }}">
        ...
        <div class="flex flex-1 min-h-0">
            ...
            <div class="flex flex-col flex-1 min-w-0">
                ...
                <div id="msg-list"
                    ...
                    @forelse($messages as $msg)
                        @php
                            ...
                            // ── SOLID PURPLE for own/coord/director — no gradient ──
                            if ($msg['is_mine']) {
                        ...
                        <div class="flex {{ $msg['is_mine'] ? 'justify-end' : 'justify-start' }} items-end gap-2 {{ $sameGroup ? 'mt-0.5' : 'mt-3' }}"
                            ...
                            {{-- Avatar – others — always show image (default if no photo) --}}
                            @if(! $msg['is_mine'])
                            <div class="w-7 h-7 rounded-full flex-shrink-0 overflow-hidden flex items-center justify-center
                                        text-xs font-semibold text-white mb-1 self-end"
                                 style="{{ $avatarGrad }}"
                                 title="{{ $msg['sender_name'] }}">
                                <img src="{{ $senderPhotoSrc }}"
                                     class="w-full h-full object-cover"
                            ...
                            <div class="flex flex-col {{ $msg['is_mine'] ? 'items-end' : 'items-start' }} max-w-[78%] sm:max-w-[70%]">
                                ...
                                <p class="text-xs font-semibold px-1 mb-0.5
                                    {{ $msg['is_director'] ? 'text-violet-700' : ($msg['is_coordinator'] ? 'text-[#6b2490]' : 'text-[#6b2490]') }}">
                                    {{ $msg['sender_name'] }}
                                    @if($msg['is_director'])
                                ...
                                <div class="text-sm rounded-lg px-2.5 py-1.5 mb-1 max-w-full border-l-[3px] leading-snug
                                    {{ $msg['is_mine']
                                        ...
                                        : 'bg-white border-[#ddd3e8] text-[#666666]' }}">
                                ...
                                <div class="flex flex-col gap-1.5 min-w-[220px]">
                                    <textarea wire:model="editBody" rows="2"
                                              class="text-sm rounded-lg border border-[#6b2490] px-3 py-2 resize-none
                                                     focus:outline-none focus:ring-2 focus:ring-[#6b2490]/30 w-full bg-white shadow-sm"
                                              wire:keydown.escape="cancelEdit"></textarea>
                                    <div class="flex gap-1.5 justify-end">
                                        <button wire:click="cancelEdit"
                                                class="text-xs px-3 py-1.5 rounded-lg border border-[#ddd3e8] text-[#666666]
                                                       hover:bg-[#f5f5f5] transition font-semibold">Cancel</button>
                                ...
                                <div x-show="open"
                                     ...
                                     x-cloak
                                     class="flex flex-wrap items-center gap-1.5 mt-2 bg-white border border-[#ddd3e8]
                                    ...
                                    <button wire:click="setReply({{ $msg['id'] }})"
                                            ...
                                            class="flex items-center gap-1 px-2 py-1 rounded-lg text-[#666666]
                                                   hover:text-[#6b2490] hover:bg-[#f2e8f9] transition text-xs font-semibold">
                                    ...
                                    <button wire:click="openReactionsPopup({{ $msg['id'] }})"
                                            ...
                                            class="flex items-center gap-1 px-2 py-1 rounded-lg transition text-xs font-semibold
                                                   {{ $reactionsPopupMsgId === $msg['id']
                                                        ? 'text-[#6b2490] bg-[#f2e8f9]'
                                                        : 'text-[#666666] hover:text-[#6b2490] hover:bg-[#f2e8f9]' }}">
                                ...
                                <div class="mt-2 bg-white border border-[#ddd3e8] rounded-2xl shadow-xl z-20 w-64 overflow-hidden"
                                     @click.stop>
                                    ...
                                    <div class="max-h-52 overflow-y-auto">
                                        ...
                                        <div class="px-3.5 py-2 border-b border-[#ddd3e8] last:border-0">
                                            ...
                                            <div class="flex items-center gap-2 py-1">
                                                <div class="w-6 h-6 rounded-full flex-shrink-0 overflow-hidden flex items-center justify-center
                                                            text-xs font-semibold text-white"
                                                     style="background: #6b2490;">
                                                    <img src="{{ $reactor['photo'] ?? $defaultAv }}"
                                                         class="w-full h-full object-cover"
                                                         onerror="this.src='{{ $defaultAv }}'"
                                                         alt="">
                                                </div>
                                                <div class="flex-1 min-w-0">
                                                    ...
                                                    <p class="text-[10px] font-medium
                                                        {{ $reactor['type'] === 'director' ? 'text-violet-700' : 'text-[#6b2490]' }}">
                                ...
                                <div class="flex gap-1 mt-1 flex-wrap {{ $msg['is_mine'] ? 'justify-end' : 'justify-start' }}">
                                    ...
                                    <button wire:click="react({{ $msg['id'] }}, '{{ $rk }}')"
                                            class="inline-flex items-center gap-0.5 text-xs px-1.5 py-0.5 rounded-full border transition-all
                                                   {{ $msg['my_reaction'] === $rk
                                                        ? 'bg-[#f2e8f9] border-[#c49bdb] text-[#6b2490] font-semibold'
                                                        : 'bg-white border-[#ddd3e8] text-[#666666] hover:border-[#c49bdb]' }}">
                            ...
                            {{-- Avatar – mine — always show image (default if no photo) --}}
                            @if($msg['is_mine'])
                            <div class="w-7 h-7 rounded-full flex-shrink-0 overflow-hidden flex items-center justify-center
                                        text-xs font-semibold text-white mb-1 self-end"
                                 style="background: #6b2490;">
                    ...
                    @empty
                        <div class="flex flex-col items-center justify-center h-full py-20 text-[#999999] select-none">
                            <div class="w-16 h-16 rounded-2xl flex items-center justify-center mb-4 bg-[#f2e8f9]">
                                <i class="fa-solid {{ $isStaffRoom ? 'fa-shield-halved' : 'fa-comments' }} text-4xl text-[#6b2490]"></i>
                            </div>
                            <p class="text-base font-semibold text-[#666666]">No messages yet</p>
                            <p class="text-sm text-[#999999] mt-1">
                                {{ $isStaffRoom ? 'Start the internal staff conversation! 👋' : 'Start the conversation with this batch! 👋' }}
                            </p>
                        </div>
                    @endforelse
                </div>
                ...
                <div wire:poll.3000ms="refreshTyping" class="flex-shrink-0">
                    ...
                    <div class="flex items-center gap-2.5 px-4 py-2 bg-[#fafafa] border-t border-[#ddd3e8]">
                        ...
                        <p class="text-xs text-[#666666] font-medium">
                            ...
                            @endphp
                            <span class="font-semibold text-[#6b2490]">
                ...
                <div class="flex items-center gap-3 px-4 py-2.5 border-t border-[#ddd3e8] bg-[#f2e8f9] flex-shrink-0">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-xs font-semibold text-[#6b2490] truncate uppercase tracking-widest">
                ...
                <div class="px-4 py-3 border-t border-[#ddd3e8] bg-white flex-shrink-0" x-data>
                    ...
                    <div class="flex items-end gap-2">
                        <div class="flex-1 relative">
                            <textarea
                                ...
                                placeholder="Message {{ $isStaffRoom ? 'Staff Chat' : ($room['name'] ?? 'group') }}… (@ to mention)"
                                rows="1"
                                ...
                                "
                                class="w-full resize-none rounded-lg border border-[#ddd3e8] bg-[#fafafa]
                    ...
                    <p class="text-xs text-[#999999] text-center mt-1.5">
                        ...
                        <span class="text-[#ddd3e8]">tap message for actions</span>
                    </p>
                </div>
            </div>
            ...
            <div class="w-72 border-l border-[#ddd3e8] flex flex-col flex-shrink-0 bg-white">
                ...
                <div class="flex items-center gap-2.5 px-4 py-3 border-b border-[#ddd3e8] flex-shrink-0 bg-[#f9f7fc]">
                    ...
                    @elseif($isStaffRoom)
                        ...
                        <p class="text-sm font-semibold text-[#333333] flex-1 uppercase tracking-wide">
                            Staff Members
                            <span class="text-xs font-semibold text-[#999999] ml-1">({{ count($staffDirectors) + count($staffCoords) }})</span>
                        </p>
                    @else
                ...
                <div class="flex-1 overflow-y-auto flex flex-col">
                        ...
                        <div class="flex items-center gap-2.5 rounded-lg px-3 py-2 mb-1 bg-violet-50 border border-violet-100">
                            <div class="relative flex-shrink-0">
                                <div class="w-8 h-8 rounded-full overflow-hidden flex items-center justify-center"
                                     style="background:#4a1d78;">
                                    <img src="{{ $dir['photo'] ?? $defaultAv }}"
                                         class="w-full h-full object-cover"
                                         onerror="this.src='{{ $defaultAv }}'"
                                         alt="">
                                </div>
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs font-medium {{ $dir['is_online'] ? 'text-emerald-600' : 'text-violet-700' }}">
                                    {{ $dir['is_online'] ? '🟢 Online' : 'Director' }}
                                </p>
                            </div>
                        </div>
                        @endforeach
                    ...
                    <div class="px-3 py-2.5 border-b border-[#ddd3e8] flex-shrink-0">
                        ...
                        <div class="relative">
                            ...
                            <input wire:model.live.debounce.300ms="memberSearch" type="text" placeholder="Search staff…"
                                   class="w-full pl-8 pr-3 py-2 text-sm rounded-lg border border-[#ddd3e8]
                                          bg-[#fafafa] focus:outline-none focus:border-[#6b2490]
                                          focus:ring-1 focus:ring-[#6b2490]/20 transition placeholder-[#999999]"/>
                        </div>
                    </div>

                    <div class="flex-1 overflow-y-auto px-3 pb-3 space-y-1 pt-2">
                        @php
                            $onlineCoords  = collect($staffCoords)->where('is_online', true)->values();
                            $offlineCoords = collect($staffCoords)->where('is_online', false)->values();
                        @endphp
                        @foreach($onlineCoords as $coord)
                        <div class="flex items-center gap-2.5 rounded-lg px-3 py-2.5 border border-[#ddd3e8]
                                    hover:border-[#c49bdb] hover:bg-[#f2e8f9] transition-all
                                    {{ $coord['is_me'] ? 'bg-[#f2e8f9] border-[#c49bdb]' : '' }}">
                            <div class="relative flex-shrink-0">
                                <div class="w-8 h-8 rounded-full overflow-hidden flex items-center justify-center"
                                     style="background: #6b2490;">
                                    <img src="{{ $coord['photo'] ?? $defaultAv }}"
                                         class="w-full h-full object-cover"
                                         onerror="this.src='{{ $defaultAv }}'"
                                         alt="">
                                </div>
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-xs font-semibold text-[#333333] truncate">
                                    {{ $coord['name'] }}
                                    @if($coord['is_me'])<span class="text-[#6b2490] font-semibold">(You)</span>@endif
                                </p>
                        ...
                        <div class="flex items-center gap-2.5 rounded-lg px-3 py-2.5 border border-[#ddd3e8]
                                    ...
                                    {{ $coord['is_me'] ? 'bg-[#f2e8f9] border-[#c49bdb] opacity-100' : '' }}">
                    ...
                    <div class="px-3 pt-3 pb-1 flex-shrink-0">
                        ...
                        <div class="flex items-center gap-2.5 rounded-lg px-3 py-2 mb-1 bg-[#f2e8f9] border border-[#ddd3e8]">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-xs font-semibold text-[#333333] truncate">
                                    {{ $coord['name'] }}
                                    @if($coord['is_me'])<span class="text-xs text-[#6b2490] font-semibold">(You)</span>@endif
                                </p>
                                <p class="text-xs text-[#6b2490] font-medium">
                    ...
                    <div class="px-3 py-2.5 border-b border-[#ddd3e8] flex-shrink-0">
                        <div class="relative">
                            <i class="fa-solid fa-magnifying-glass absolute left-3 top-1/2 -translate-y-1/2 text-[#999999] text-xs pointer-events-none"></i>
                            <input wire:model.live.debounce.300ms="memberSearch" type="text" placeholder="Search alumni…"
                                   class="w-full pl-8 pr-3 py-2 text-sm rounded-lg border border-[#ddd3e8]
                                          bg-[#fafafa] focus:outline-none focus:border-[#6b2490]
                                          focus:ring-1 focus:ring-[#6b2490]/20 transition placeholder-[#999999]"/>
                        </div>
