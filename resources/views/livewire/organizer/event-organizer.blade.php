...
<div
    ...
    <div class="flex-1 min-w-0">
        ...
        <p class="text-sm mt-1 opacity-80 leading-snug break-words" x-text="msg"></p>
    </div>
...
<div class="flex flex-col px-4 sm:px-6 lg:px-8 pt-6 pb-8 max-w-screen-2xl mx-auto min-h-screen bg-gray-50">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div class="flex items-center gap-4">
            <div class="w-14 h-14 sm:w-16 sm:h-16 rounded-xl bg-[#7a3f91] flex items-center justify-center shadow-lg flex-shrink-0" style="box-shadow:0 4px 14px rgba(122,63,145,.35);">
                <i class="fas fa-calendar-days text-white text-xl sm:text-2xl"></i>
            </div>
            <div>
                <h1 class="text-3xl sm:text-4xl font-bold text-gray-800 tracking-tight">Event Organizer</h1>
        ...
        <button wire:click="openCreateModal"
                class="inline-flex items-center gap-2 px-6 py-3 text-white text-base font-bold rounded-xl shadow-md transition flex-shrink-0 {{ !$this->hasAlumni ? 'opacity-60 cursor-not-allowed' : '' }}"
                style="background:#7a3f91;" onmouseover="this.style.background='#5e2f72'" onmouseout="this.style.background='#7a3f91'">
            <i class="fas fa-plus text-sm"></i> Post Event
        </button>
    </div>

    {{-- No alumni notice banner --}}
    @if(!$this->hasAlumni)
    <div class="mb-4 flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl px-5 py-4 text-sm text-amber-800">
        <i class="fas fa-triangle-exclamation text-amber-500 mt-0.5 flex-shrink-0 text-base"></i>
        <div>
            <p class="font-bold text-base">No verified alumni found for {{ $this->organizerDepartment ?: 'your college' }}.</p>
            <p class="text-sm mt-1 text-amber-700">You cannot post events until at least one verified alumni is registered under your college.</p>
        </div>
    </div>
    @endif
    ...
    <div class="bg-white rounded-2xl shadow-md border border-gray-100 flex flex-col overflow-hidden" style="min-height:0;height:calc(100vh - {{ $this->hasAlumni ? '210px' : '270px' }});">
        ...
        <div class="px-4 sm:px-6 py-3 border-b border-gray-100 bg-white flex flex-wrap gap-2 items-center">
            <div class="relative flex-1 min-w-[160px] sm:min-w-[200px] max-w-sm"
                ...
                <input type="text" x-model="q" @input.debounce.400ms="$wire.set('search',q)"
                       ...
                       class="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-lg text-sm bg-white text-gray-800 transition focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100"
                       autocomplete="off">
            </div>
            <select wire:model.live="filterStatus" class="px-4 py-2.5 border border-gray-200 rounded-lg text-sm bg-white text-gray-700 focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100 transition">
            ...
            <select wire:model.live="filterSort" class="px-4 py-2.5 border border-gray-200 rounded-lg text-sm bg-white text-gray-700 focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100 transition hidden sm:block">
                <option value="recent">Recent First</option>
                <option value="oldest">Oldest First</option>
            </select>
            <button wire:click="resetFilters" class="px-4 py-2.5 rounded-lg border border-gray-200 bg-white text-sm font-medium text-gray-600 hover:bg-gray-50 transition flex items-center gap-1.5">
                <i class="fas fa-rotate-left text-sm"></i><span class="hidden sm:inline">Reset</span>
            </button>
        </div>

        {{-- Mobile row 2 --}}
        <div class="px-4 py-2 border-b border-gray-100 bg-white flex gap-2 sm:hidden">
            <select wire:model.live="filterSort" class="flex-1 px-4 py-2.5 border border-gray-200 rounded-lg text-sm bg-white text-gray-700 focus:outline-none focus:border-purple-400 transition">
                <option value="recent">Recent First</option>
                <option value="oldest">Oldest First</option>
            </select>
        </div>
                ...
                <table class="w-full border-collapse min-w-[700px]">
                    <thead>
                        <tr class="bg-gray-100 border-b border-gray-200 sticky top-0 z-10">
                            <th class="px-5 py-3.5 text-left text-sm font-semibold text-gray-600 uppercase tracking-wider">Event</th>
                            <th class="px-5 py-3.5 text-left text-sm font-semibold text-gray-600 uppercase tracking-wider">Date & Time</th>
                            <th class="px-5 py-3.5 text-left text-sm font-semibold text-gray-600 uppercase tracking-wider hidden md:table-cell">Courses</th>
                            <th class="px-5 py-3.5 text-center text-sm font-semibold text-gray-600 uppercase tracking-wider hidden lg:table-cell">RSVPs</th>
                            <th class="px-5 py-3.5 text-center text-sm font-semibold text-gray-600 uppercase tracking-wider">Status</th>
                            <th class="px-5 py-3.5 text-center text-sm font-semibold text-gray-600 uppercase tracking-wider">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-50">
                        ...
                        <tr class="bg-white hover:bg-gray-50 transition-colors duration-100">
                            ...
                            <td class="px-5 py-3.5 max-w-[180px] sm:max-w-[220px]">
                                <p class="font-semibold text-base truncate text-gray-800">{{ $event->title }}</p>
                                @if(!empty($parts[1]))
                                    <p class="text-sm mt-1 text-gray-500">Batch {{ trim($parts[1]) }}</p>
                                @endif
                            </td>

                            <td class="px-5 py-3.5 whitespace-nowrap">
                                <span class="text-base font-semibold text-gray-700">{{ $event->event_date->setTimezone('Asia/Manila')->format('M d, Y') }}</span>
                            ...
                            <td class="px-5 py-3.5 hidden md:table-cell">
                                <p class="text-sm font-medium max-w-[150px] truncate text-gray-600" title="{{ $displayCourses }}">{{ $displayCourses }}</p>
                            </td>

                            <td class="px-5 py-3.5 text-center hidden lg:table-cell">
                                <div class="flex items-center justify-center gap-1.5">
                                    <span class="relative group">
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm font-bold">
                                            <i class="fas fa-circle-check text-[10px]"></i>{{ $event->confirmed_count }}
                                        </span>
                                        <span class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1.5 bg-gray-800 text-white text-xs font-semibold rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition pointer-events-none z-20">Confirmed<span class="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-gray-800"></span></span>
                                    </span>
                                    <span class="relative group">
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-red-50 border border-red-200 text-red-600 text-sm font-bold">
                                    ...
                                    <span class="relative group">
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-amber-50 border border-amber-200 text-amber-600 text-sm font-bold">
                            ...
                            <td class="px-5 py-3.5 text-center whitespace-nowrap">
                                @if($isCompleted)
                                    <span class="inline-flex items-center gap-1 px-3 py-1 bg-green-100 text-green-800 border border-green-300 rounded-full text-xs font-bold">
                                        <i class="fas fa-circle-check text-[10px]"></i> Completed
                                    </span>
                                @elseif($isApproved)
                                    <span class="inline-block px-3 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-full text-xs font-bold">Approved</span>
                                @elseif($event->status === 'PENDING')
                                    <span class="inline-block px-3 py-1 bg-yellow-50 text-yellow-700 border border-yellow-200 rounded-full text-xs font-bold">Pending</span>
                                @elseif($event->status === 'ORGANIZER_DELETED')
                                    <span class="inline-block px-3 py-1 bg-red-100 text-red-700 border border-red-300 rounded-full text-xs font-bold">Deleted</span>
                                @else
                                    <span class="inline-block px-3 py-1 bg-red-50 text-red-700 border border-red-200 rounded-full text-xs font-bold">Rejected</span>
                                @endif
                            </td>
                            ...
                            <td class="px-5 py-3.5 text-center">
                                <div class="flex items-center justify-center gap-2 flex-wrap">
                                    {{-- VIEW --}}
                                    <button wire:click="viewEvent({{ $event->id }})" class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-purple-700 bg-purple-50 border border-purple-200 hover:bg-purple-100 rounded-lg transition">
                                    ...
                                    @if($isApproved)
                                        <button wire:click="openShareModal({{ $event->id }})"
                                                ...
                                                class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-sky-700 bg-sky-50 border border-sky-200 hover:bg-sky-100 rounded-lg transition">
                                            <i class="fas fa-share-nodes text-xs"></i><span class="hidden sm:inline">Share</span>
                                        </button>
                                    @elseif($isCompleted)
                                        <button wire:click="openShareModal({{ $event->id }})"
                                                title="Share event highlights"
                                                class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-green-700 bg-green-50 border border-green-300 hover:bg-green-100 rounded-lg transition">
                                            <i class="fas fa-star text-xs"></i><span class="hidden sm:inline">Highlights</span>
                                        </button>
                                    @endif

                                    {{-- EDIT / DELETE — only for non-deleted, non-completed, non-approved --}}
                                    @if(!$isDeleted && !$isCompleted && !$isApproved)
                                    <button wire:click="openEditModal({{ $event->id }})" class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-blue-600 bg-white border border-blue-200 hover:bg-blue-50 rounded-lg transition">
                                        <i class="fas fa-pen-to-square text-xs"></i><span class="hidden sm:inline">Edit</span>
                                    </button>
                                    <button wire:click="confirmDelete({{ $event->id }})" class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-red-600 bg-white border border-red-200 hover:bg-red-50 rounded-lg transition">
                            ...
                            <td colspan="6" class="py-20 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    <div class="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center">
                                        <i class="fas fa-calendar-days text-3xl text-gray-300"></i>
                                    </div>
                                    <p class="font-semibold text-gray-400 text-lg">No events found</p>
        ...
        {{-- Pagination --}}
        <div class="px-5 py-3.5 border-t border-gray-100 bg-[#2b0d3e] shrink-0">
            @php
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                <p class="text-white text-sm sm:text-base">Showing <span class="font-bold text-white">{{ $from }}–{{ $to }}</span> of <span class="font-bold text-white">{{ $total }}</span> events</p>
                <div class="flex items-center gap-2">
                    @if($this->events->onFirstPage())
                        <button disabled class="px-4 sm:px-5 py-2 bg-gray-100 text-gray-400 rounded-lg text-sm sm:text-base font-semibold cursor-not-allowed">← Prev</button>
                    @else
                        <button wire:click="previousPage" class="px-4 sm:px-5 py-2 text-white rounded-lg text-sm sm:text-base font-semibold transition" style="background:#7a3f91;" onmouseover="this.style.background='#5e2f72'" onmouseout="this.style.background='#7a3f91'">← Prev</button>
                    @endif
                    <span class="px-4 py-2 bg-white border border-gray-200 rounded-lg text-gray-600 text-sm sm:text-base font-semibold shadow-sm">{{ $cp }} / {{ $this->events->lastPage() }}</span>
                    @if($this->events->hasMorePages())
                        <button wire:click="nextPage" class="px-4 sm:px-5 py-2 text-white rounded-lg text-sm sm:text-base font-semibold transition" style="background:#7a3f91;" onmouseover="this.style.background='#5e2f72'" onmouseout="this.style.background='#7a3f91'">Next →</button>
                    @else
                        <button disabled class="px-4 sm:px-5 py-2 bg-gray-100 text-gray-400 rounded-lg text-sm sm:text-base font-semibold cursor-not-allowed">Next →</button>
                    @endif
                </div>
            </div>
        </div>
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" @keydown.escape.window="$wire.closeNoAlumniModal()">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden"
        ...
        <div class="p-6">
            <p class="text-gray-600 text-base mb-2">No verified alumni found for:</p>
            ...
            <button wire:click="closeNoAlumniModal" class="w-full px-4 py-3 border border-gray-200 text-gray-700 rounded-xl text-base font-bold hover:bg-gray-50 transition">Close</button>
        </div>
    </div>
</div>
@endif
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/50 backdrop-blur-sm" @keydown.escape.window="$wire.closeFormModal()">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[95vh] sm:max-h-[92vh] flex flex-col overflow-hidden"
        ...
        <div class="flex items-center justify-between px-6 sm:px-8 py-5 text-white flex-shrink-0" style="background:#7a3f91;">
            <div>
        ...
        <div class="flex-1 min-h-0 overflow-y-auto px-6 sm:px-8 py-6 space-y-6" x-ref="formScroll" style="scrollbar-width:thin;scrollbar-color:#d1d5db #f3f4f6;">

            <div>
                <label class="block text-sm font-semibold text-gray-600 uppercase tracking-wide mb-2">Event Photo <span class="text-gray-400 font-normal normal-case text-xs">(Optional)</span></label>
                <div x-data="{isDragging:false}" @dragover.prevent="isDragging=true" @dragleave.prevent="isDragging=false" @drop.prevent="isDragging=false"
                    ...
                    <label class="cursor-pointer block">
                        ...
                        @if($photo)
                            <div class="flex flex-col items-center gap-3"><img src="{{ $photo->temporaryUrl() }}" class="w-32 h-24 object-cover rounded-xl shadow border border-purple-200"><p class="text-sm font-semibold text-purple-600"><i class="fas fa-check-circle mr-1"></i>New photo selected</p></div>
                        ...
                        @else
                            <div class="flex flex-col items-center gap-2 py-2"><i class="fas fa-cloud-arrow-up text-4xl text-gray-300"></i><p class="font-semibold text-gray-500 text-base">Click to upload or drag & drop</p><p class="text-sm text-gray-400">JPG, PNG, WEBP — max 5MB</p><p class="text-sm text-purple-500 font-medium mt-1"><i class="fas fa-image mr-1"></i>Default photo will be used if blank</p></div>
                        @endif
                    </label>
                </div>
                @if($existingPhotoUrl&&!$removePhoto&&!$photo)
                    <div class="mt-2 flex items-center gap-2">
                        <button type="button" wire:click="$set('removePhoto',true)" class="text-sm text-red-600 hover:text-red-700 font-semibold flex items-center gap-1.5 px-3 py-2 rounded-lg border border-red-200 hover:bg-red-50 transition"><i class="fas fa-trash text-xs"></i> Remove photo</button>
                        <span class="text-sm text-gray-400">(uses default)</span>
                    </div>
                @endif
                @if($removePhoto)
                    <div class="mt-2 flex items-center gap-2"><span class="text-sm text-amber-700 font-semibold"><i class="fas fa-exclamation-circle mr-1"></i>Photo will be removed on save</span><button type="button" wire:click="$set('removePhoto',false)" class="text-sm text-blue-600 underline">Undo</button></div>
                @endif
                <div wire:loading wire:target="photo" class="mt-2 text-sm text-purple-700 flex items-center gap-2"><i class="fas fa-spinner animate-spin"></i> Uploading…</div>
            </div>
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden">
                <div class="bg-gray-50 px-4 py-3 border-b border-gray-200 flex items-center gap-2">
                    ...
                    <span class="text-base font-semibold text-gray-700">Event Details</span>
                </div>
                <div class="p-6 space-y-5">
                    <div>
                        <label class="block text-sm font-semibold text-gray-600 uppercase tracking-wide mb-2">Event Title <span class="text-red-500">*</span></label>
                        <input wire:model.defer="title" type="text" placeholder="e.g. PHILCST Alumni Homecoming 2026"
                               class="w-full px-4 py-3 border rounded-lg text-base text-gray-800 bg-white transition focus:outline-none focus:ring-2 {{ isset($formErrors['title'])?'border-red-400 bg-red-50 focus:border-red-400 focus:ring-red-100':'border-gray-200 focus:border-purple-400 focus:ring-purple-100' }}">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-600 uppercase tracking-wide mb-2">Description <span class="text-red-500">*</span></label>
                        <textarea wire:model.defer="description" rows="4" placeholder="Describe the event, agenda, highlights…"
                                  class="w-full px-4 py-3 border rounded-lg text-base text-gray-800 bg-white focus:outline-none focus:ring-2 transition resize-none {{ isset($formErrors['description'])?'border-red-400 bg-red-50 focus:border-red-400 focus:ring-red-100':'border-gray-200 focus:border-purple-400 focus:ring-purple-100' }}"></textarea>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-600 uppercase tracking-wide mb-2">Event Date <span class="text-red-500">*</span></label>
                        <input wire:model="event_date" type="date" min="{{ now('Asia/Manila')->format('Y-m-d') }}"
                               class="w-full px-4 py-3 border rounded-lg text-base text-gray-800 bg-white transition focus:outline-none focus:ring-2 {{ isset($formErrors['event_date'])?'border-red-400 bg-red-50 focus:border-red-400 focus:ring-red-100':'border-gray-200 focus:border-purple-400 focus:ring-purple-100' }}">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-sm font-semibold text-gray-600 uppercase tracking-wide mb-2">Start Time <span class="text-red-500">*</span></label>
                            <input wire:model="start_time" type="text" placeholder="e.g. 8:00 AM"
                                   class="w-full px-4 py-3 border rounded-lg text-base text-gray-800 bg-white transition focus:outline-none focus:ring-2 {{ isset($formErrors['start_time'])?'border-red-400 bg-red-50 focus:border-red-400 focus:ring-red-100':'border-gray-200 focus:border-purple-400 focus:ring-purple-100' }}">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-600 uppercase tracking-wide mb-2">End Time</label>
                            <input wire:model="end_time" type="text" placeholder="e.g. 5:00 PM"
                                   class="w-full px-4 py-3 border rounded-lg text-base text-gray-800 bg-white transition focus:outline-none focus:ring-2 {{ isset($formErrors['end_time'])?'border-red-400 bg-red-50 focus:border-red-400 focus:ring-red-100':'border-gray-200 focus:border-purple-400 focus:ring-purple-100' }}">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-sm font-semibold text-gray-600 uppercase tracking-wide mb-2">Venue / Location <span class="text-red-500">*</span></label>
                            <input wire:model.defer="venue" type="text" placeholder="e.g. PHILCST Main Gym"
                                   class="w-full px-4 py-3 border rounded-lg text-base text-gray-800 bg-white transition focus:outline-none focus:ring-2 {{ isset($formErrors['venue'])?'border-red-400 bg-red-50 focus:border-red-400 focus:ring-red-100':'border-gray-200 focus:border-purple-400 focus:ring-purple-100' }}">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-600 uppercase tracking-wide mb-2">Full Address</label>
                            <input wire:model.defer="venue_address" type="text"
                                   placeholder="e.g. Old Nalsian Road, Nalsian, Calasiao, Pangasinan"
                                   class="w-full px-4 py-3 border border-gray-200 rounded-lg text-base text-gray-800 bg-white focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100 transition">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden">
                <div class="bg-gray-50 px-4 py-3 border-b border-gray-200 flex items-center gap-2">
                    ...
                    <span class="text-base font-semibold text-gray-700">Courses / Programs</span>
                </div>
                <div class="p-6 space-y-5">
                    <div class="flex items-center gap-3 bg-purple-50 border border-purple-200 rounded-xl px-4 py-3">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="text-base font-semibold text-purple-800">{{ $this->organizerDepartment ?: 'Your College' }}</div>
                            <div class="text-sm text-purple-700 mt-0.5">Select specific courses or leave unchecked for all courses in your college.</div>
                        </div>
                    </div>
                    ...
                    @if(count($this->availableCourses) > 0)
                        <div>
                            <div class="flex items-center justify-between mb-3">
                                <span class="text-sm font-semibold text-gray-600 uppercase tracking-wide">Available Courses</span>
                                <div class="flex gap-3">
                                    <button type="button" wire:click="$set('selectedCourses', {{ json_encode($this->availableCourses) }})" class="text-sm text-purple-700 font-bold hover:underline"><i class="fas fa-check-double mr-1"></i>Select All</button>
                                    @if(count($selectedCourses) > 0)<button type="button" wire:click="$set('selectedCourses', [])" class="text-sm text-gray-500 hover:text-red-600 font-bold hover:underline">Clear</button>@endif
                                </div>
                            </div>
                            ...
                            @if(count($selectedCourses) > 0)
                                <div class="mt-3 flex flex-wrap gap-2">
                                    @foreach($selectedCourses as $course)
                                        <span class="inline-flex items-center gap-2 px-3 py-1.5 bg-purple-50 border border-purple-200 text-purple-700 text-sm font-semibold rounded-lg"><i class="fas fa-book text-xs"></i>{{ $course }}</span>
                                    @endforeach
                                </div>
                            @endif
                        </div>
                    @else
                        <div class="text-center py-6 text-gray-400 text-base">
                    ...
                    <div class="pt-3 border-t border-gray-100">
                        <label class="block text-sm font-semibold text-gray-600 uppercase tracking-wide mb-2">
                            Batch Year <span class="text-gray-500 font-normal normal-case text-xs">(Optional — leave blank for all batches)</span>
                        </label>
                        <input wire:model.defer="batchYear" type="number" min="1990" max="{{ now()->year + 5 }}" placeholder="e.g. {{ now()->year - 2 }}"
                               class="w-full sm:max-w-xs px-4 py-3 border rounded-lg text-base text-gray-800 bg-white transition focus:outline-none focus:ring-2 {{ isset($formErrors['batch_year'])?'border-red-400 bg-red-50 focus:border-red-400 focus:ring-red-100':'border-gray-200 focus:border-purple-400 focus:ring-purple-100' }}">
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden">
                <div class="bg-gray-50 px-4 py-3 border-b border-gray-200 flex items-center gap-2">
                    ...
                    <span class="text-base font-semibold text-gray-700">Contact Person</span>
                    <span class="text-xs text-gray-500 font-normal ml-1">— pre-filled from your account</span>
                </div>
                <div class="p-6 grid grid-cols-1 sm:grid-cols-3 gap-5">
                    <div>
                        <label class="block text-sm font-semibold text-gray-600 uppercase tracking-wide mb-2">Name</label>
                        <input wire:model.defer="contact_person" type="text" placeholder="Full name"
                               class="w-full px-4 py-3 border border-gray-200 rounded-lg text-base text-gray-800 bg-white focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100 transition">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-600 uppercase tracking-wide mb-2">Email</label>
                        <input wire:model.defer="contact_email" type="email" placeholder="contact@example.com"
                               class="w-full px-4 py-3 border rounded-lg text-base text-gray-800 bg-white transition focus:outline-none focus:ring-2 {{ isset($formErrors['contact_email'])?'border-red-400 bg-red-50 focus:border-red-400 focus:ring-red-100':'border-gray-200 focus:border-purple-400 focus:ring-purple-100' }}">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-600 uppercase tracking-wide mb-2">Phone <span class="text-gray-500 font-normal normal-case text-xs">(Optional)</span></label>
                        <input wire:model.defer="contact_phone" type="text" placeholder="+63 9XX XXX XXXX"
                               class="w-full px-4 py-3 border border-gray-200 rounded-lg text-base text-gray-800 bg-white focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100 transition">
                    </div>
                    <div class="col-span-1 sm:col-span-3">
                        <p class="text-sm text-gray-500"><i class="fas fa-circle-info text-xs mr-1"></i>Name and email are pre-filled from your account. You may update them if needed.</p>
                    </div>
                </div>
            </div>

            <div>
                <label class="block text-sm font-semibold text-gray-600 uppercase tracking-wide mb-2">Additional Notes / Requirements</label>
                <textarea wire:model.defer="notes" rows="4" placeholder="Dress code, special instructions…"
                          class="w-full px-4 py-3 border border-gray-200 rounded-lg text-base text-gray-800 bg-white focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100 transition resize-none"></textarea>
            </div>
        ...
        <div class="px-6 sm:px-8 py-4 border-t border-gray-100 bg-gray-50/70 flex-shrink-0 flex gap-3">
            <button type="button" wire:click="closeFormModal" class="flex-1 px-4 py-3 border border-gray-200 text-gray-700 rounded-xl text-base font-bold hover:bg-gray-100 transition">Cancel</button>
            <button type="button" wire:click="saveEvent" wire:loading.attr="disabled" wire:target="saveEvent"
                    class="flex-1 px-4 py-3 text-white rounded-xl text-base font-bold flex items-center justify-center gap-2 transition shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
                    style="background:#7a3f91;" onmouseover="this.style.background='#5e2f72'" onmouseout="this.style.background='#7a3f91'">
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/50 backdrop-blur-sm" @keydown.escape.window="$wire.closeViewModal()">
    <div class="bg-white rounded-2xl shadow-2xl flex flex-col w-full max-w-2xl max-h-[95vh] sm:max-h-[92vh] overflow-hidden relative"
         ...
         x-transition:enter-start="opacity-0 scale-95 translate-y-2"
         x-transition:enter-end="opacity-100 scale-100 translate-y-0">
        <button wire:click="closeViewModal" class="absolute top-4 right-4 z-10 w-9 h-9 rounded-full bg-black/40 hover:bg-black/60 text-white flex items-center justify-center text-xl leading-none transition">×</button>

        <img src="{{ $ev->photo_url }}" alt="{{ $ev->title }}" class="w-full h-48 sm:h-72 object-cover flex-shrink-0 {{ $isCompleted ? 'brightness-90' : '' }}">

        <div class="px-6 sm:px-8 pt-6 pb-5 border-b border-gray-100 flex-shrink-0">
            <div class="flex items-start justify-between gap-3 mb-4">
                <h2 class="text-xl sm:text-2xl font-bold leading-snug text-gray-900">{{ $ev->title }}</h2>
                @if($isCompleted)
                    <span class="flex-shrink-0 inline-flex items-center gap-1 px-3 py-1.5 bg-green-100 text-green-800 border border-green-300 rounded-full text-xs font-bold">
                        <i class="fas fa-circle-check text-[10px]"></i> Completed
                    </span>
                @elseif($isApproved)
                    <span class="flex-shrink-0 px-3 py-1.5 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-full text-xs font-bold">Approved</span>
                @elseif($ev->status==='PENDING')
                    <span class="flex-shrink-0 px-3 py-1.5 bg-yellow-50 text-yellow-700 border border-yellow-200 rounded-full text-xs font-bold">Pending</span>
                @else
                    <span class="flex-shrink-0 px-3 py-1.5 bg-red-50 text-red-700 border border-red-200 rounded-full text-xs font-bold">Rejected</span>
                @endif
            </div>
            <ul class="space-y-2">
                <li class="flex items-start gap-3 text-base text-gray-700"><i class="fas fa-calendar" style="color:#7a3f91" class="mt-0.5 w-5 flex-shrink-0"></i><span>{{ $ev->event_date->setTimezone('Asia/Manila')->format('F d, Y') }}</span></li>
                <li class="flex items-start gap-3 text-base text-gray-700"><i class="fas fa-clock" style="color:#7a3f91" class="mt-0.5 w-5 flex-shrink-0"></i>
                    <span>{{ $ev->event_date->setTimezone('Asia/Manila')->format('g:i A') }}@if($ev->event_end_date)<span class="text-gray-500 mx-1">–</span>{{ $ev->event_end_date->setTimezone('Asia/Manila')->format('g:i A') }}@else<span class="text-gray-500 italic ml-1">· End time not set</span>@endif</span>
                </li>
                <li class="flex items-start gap-3 text-base text-gray-700"><i class="fas fa-location-dot" style="color:#7a3f91" class="mt-0.5 w-5 flex-shrink-0"></i><span>{{ $ev->venue }}@if($ev->venue_address) · <span class="text-gray-600">{{ $ev->venue_address }}</span>@endif</span></li>
                @if($ev->target_participants)<li class="flex items-start gap-3 text-base text-gray-700"><i class="fas fa-book" style="color:#7a3f91" class="mt-0.5 w-5 flex-shrink-0"></i><span class="font-semibold text-purple-600">{{ $ev->target_participants }}</span></li>@endif
            </ul>
            <p class="text-sm text-gray-500 mt-3">Posted {{ $ev->created_at->diffForHumans() }}</p>
        </div>
        ...
        <div class="flex-1 min-h-0 overflow-y-auto" style="scrollbar-width:thin;scrollbar-color:#d1d5db #f3f4f6;">
            <div class="px-6 sm:px-8 py-6 border-b border-gray-100">
                <h3 class="text-base font-bold text-gray-800 mb-3">Attendee Responses @if($totalRsvp>0)<span class="text-gray-500 font-normal text-sm ml-1">{{ $totalRsvp }} total</span>@endif</h3>
                @if($totalRsvp===0)
                    <div class="text-center py-8 text-gray-400 text-base"><i class="fas fa-inbox text-3xl block mb-2 text-gray-200"></i>No responses yet.</div>
                @else
                    <div class="grid grid-cols-3 gap-3">
                        <div class="bg-emerald-50 border border-emerald-200 rounded-xl p-4 text-center"><i class="fas fa-circle-check text-emerald-500 text-2xl mb-1"></i><div class="text-3xl font-black text-emerald-700">{{ $ev->confirmed_count }}</div><div class="text-xs font-bold text-emerald-600 uppercase tracking-wide mt-1">Confirmed</div></div>
                        <div class="bg-red-50 border border-red-200 rounded-xl p-4 text-center"><i class="fas fa-circle-xmark text-red-500 text-2xl mb-1"></i><div class="text-3xl font-black text-red-700">{{ $ev->declined_count }}</div><div class="text-xs font-bold text-red-600 uppercase tracking-wide mt-1">Not Attending</div></div>
                        <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 text-center"><i class="fas fa-circle-question text-amber-500 text-2xl mb-1"></i><div class="text-3xl font-black text-amber-700">{{ $ev->tentative_count }}</div><div class="text-xs font-bold text-amber-600 uppercase tracking-wide mt-1">Maybe</div></div>
                    </div>
                @endif
            </div>

            <div class="px-6 sm:px-8 py-6 border-b border-gray-100">
                <h3 class="text-base font-bold text-gray-800 mb-3">Event Status</h3>
                @if($isCompleted)
                    <div class="bg-green-50 border border-green-200 rounded-xl px-4 py-3">
                        <p class="text-base font-bold text-green-800"><i class="fas fa-circle-check mr-2 text-green-600"></i>Event Completed</p>
                        <p class="text-sm text-green-700 mt-1">This event has already taken place. Thank you for a successful event!</p>
                    </div>
                @elseif($isApproved)
                    <div class="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3"><p class="text-base font-bold text-emerald-800"><i class="fas fa-circle-check mr-2 text-emerald-600"></i>Approved — Now Live</p>@if($ev->reviewed_at)<p class="text-sm text-emerald-700 mt-1">{{ $ev->reviewed_at->setTimezone('Asia/Manila')->format('M d, Y · g:i A') }}</p>@endif@if($ev->review_remarks)<p class="text-sm text-emerald-700 mt-1 italic">"{{ $ev->review_remarks }}"</p>@endif</div>
                @elseif($ev->status==='PENDING')
                    <div class="bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-3"><p class="text-base font-bold text-yellow-800"><i class="fas fa-hourglass-half mr-2 text-yellow-600"></i>Awaiting Admin Review</p><p class="text-sm text-yellow-700 mt-1">Your event is pending approval. You'll be notified once reviewed.</p></div>
                @else
                    <div class="bg-red-50 border border-red-200 rounded-xl px-4 py-3"><p class="text-base font-bold text-red-800"><i class="fas fa-circle-xmark mr-2 text-red-600"></i>Rejected by Admin</p>@if($ev->review_remarks)<p class="text-sm text-red-700 mt-2"><span class="font-semibold">Reason:</span> {{ $ev->review_remarks }}</p>@endif<p class="text-sm text-red-700 mt-2 font-semibold">You may edit and resubmit this event for re-review.</p></div>
                @endif
            </div>

            @if($ev->description)<div class="px-6 sm:px-8 py-6 border-b border-gray-100"><h3 class="text-base font-bold text-gray-800 mb-3">About This Event</h3><p class="text-base text-gray-700 leading-relaxed whitespace-pre-wrap">{{ $ev->description }}</p></div>@endif
            @if($ev->notes)<div class="px-6 sm:px-8 py-6 border-b border-gray-100"><h3 class="text-base font-bold text-gray-800 mb-3">Additional Notes</h3><p class="text-base text-gray-700 leading-relaxed whitespace-pre-wrap">{{ $ev->notes }}</p></div>@endif

            @if($ev->contact_person||$ev->contact_email||$ev->contact_phone)
            <div class="px-6 sm:px-8 py-6 border-b border-gray-100">
                <h3 class="text-base font-bold text-gray-800 mb-3">Contact Person</h3>
                <div class="space-y-2 text-base text-gray-700">
                    @if($ev->contact_person)<p><i class="fas fa-user" style="color:#7a3f91" class="w-5 mr-2"></i>{{ $ev->contact_person }}</p>@endif
                    @if($ev->contact_email)<p><i class="fas fa-envelope" style="color:#7a3f91" class="w-5 mr-2"></i>{{ $ev->contact_email }}</p>@endif
                    @if($ev->contact_phone)<p><i class="fas fa-phone" style="color:#7a3f91" class="w-5 mr-2"></i>{{ $ev->contact_phone }}</p>@endif
                </div>
            </div>
            @endif

            <div class="px-6 sm:px-8 py-6">
                <p class="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">Posting Details</p>
                <div class="grid grid-cols-2 sm:grid-cols-3 border border-gray-100 rounded-xl overflow-hidden divide-x divide-y divide-gray-100">
                    <div class="px-4 py-3"><p class="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">Submitted</p><p class="text-sm font-semibold text-gray-800">{{ $ev->created_at->setTimezone('Asia/Manila')->format('M d, Y') }}</p><p class="text-xs text-gray-500">{{ $ev->created_at->setTimezone('Asia/Manila')->format('g:i A') }}</p></div>
                    <div class="px-4 py-3"><p class="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">Last Updated By</p><p class="text-sm font-semibold text-gray-800">{{ $ev->updated_by ?? 'System' }}</p><p class="text-xs text-gray-500">{{ $ev->updated_by_role ? ucfirst($ev->updated_by_role) : '—' }}</p></div>
                    <div class="px-4 py-3 col-span-2 sm:col-span-1"><p class="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">Status</p>
        ...
        <div class="px-6 sm:px-8 py-4 border-t border-gray-100 flex items-center justify-end gap-2 flex-wrap bg-white flex-shrink-0">
            <button wire:click="closeViewModal" class="inline-flex items-center gap-1.5 px-4 py-2.5 text-base font-bold text-gray-600 border border-gray-200 bg-white hover:bg-gray-50 rounded-xl transition"><i class="fas fa-xmark text-xs"></i> Close</button>

            {{-- Share button — APPROVED: "Share", COMPLETED: "Share Highlights" --}}
            @if($isApproved)
                <button wire:click="openShareModal({{ $ev->id }})"
                        class="inline-flex items-center gap-1.5 px-4 py-2.5 text-base font-bold text-sky-700 border border-sky-200 bg-sky-50 hover:bg-white hover:border-sky-400 rounded-xl transition">
                    <i class="fas fa-share-nodes text-xs"></i> Share
                </button>
            @elseif($isCompleted)
                <button wire:click="openShareModal({{ $ev->id }})"
                        class="inline-flex items-center gap-1.5 px-4 py-2.5 text-base font-bold text-green-700 border border-green-300 bg-green-50 hover:bg-white hover:border-green-500 rounded-xl transition">
                    <i class="fas fa-star text-xs"></i> Share Highlights
                </button>
            @endif

            @if(!$isCompleted && !$isApproved && $ev->status !== 'ORGANIZER_DELETED')
            <button wire:click="confirmDelete({{ $ev->id }})" class="inline-flex items-center gap-1.5 px-4 py-2.5 text-base font-bold text-red-600 border border-red-200 bg-white hover:bg-red-50 rounded-xl transition"><i class="fas fa-trash text-xs"></i> Delete</button>
            <button wire:click="openEditModal({{ $ev->id }})" class="inline-flex items-center gap-1.5 px-4 py-2.5 text-base font-bold text-blue-600 border border-blue-200 bg-white hover:bg-blue-50 rounded-xl transition"><i class="fas fa-pen-to-square text-xs"></i> Edit</button>
            @endif
        </div>
    </div>
</div>
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" @keydown.escape.window="$wire.cancelDelete()">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden"
         x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95 translate-y-2" x-transition:enter-end="opacity-100 scale-100 translate-y-0">
        <div class="px-6 py-5 bg-red-50 border-b border-red-100"><h2 class="text-lg font-bold text-red-800 flex items-center gap-2.5"><div class="w-8 h-8 bg-red-100 rounded-xl flex items-center justify-center"><i class="fas fa-triangle-exclamation text-red-500 text-sm"></i></div>Delete Event</h2></div>
        <div class="p-6">
            <p class="text-gray-600 text-base mb-1">You are about to delete:</p>
            <p class="font-bold text-red-700 text-lg mb-4">"{{ $deleteEventTitle }}"</p>
            <div class="bg-red-50 border border-red-100 rounded-xl px-4 py-3 mb-5 text-sm text-gray-700 flex items-start gap-2"><i class="fas fa-exclamation-circle text-red-500 mt-0.5 shrink-0"></i><span>This event will be removed from your list. <strong>Admin can still see and restore it</strong> if needed.</span></div>
            <div class="flex gap-3">
                <button wire:click="cancelDelete" class="flex-1 px-4 py-3 border border-gray-200 text-gray-700 rounded-xl text-base font-bold hover:bg-gray-50 transition">Cancel</button>
                <button wire:click="executeDelete" wire:loading.attr="disabled" wire:target="executeDelete" class="flex-1 px-4 py-3 bg-red-600 hover:bg-red-700 disabled:bg-red-300 text-white rounded-xl text-base font-bold flex items-center justify-center gap-2 transition shadow-md">
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden max-h-[92vh] flex flex-col"
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
            <h2 class="text-lg font-bold text-gray-800 flex items-center gap-2">
                @if($isCompleted)
                    <i class="fas fa-star text-green-600 text-base"></i> Share Event Highlights
                @else
                    <i class="fas fa-share-nodes text-sky-600 text-base"></i> Share Event
                @endif
            </h2>
        ...
        <div class="flex-1 overflow-y-auto px-6 pt-6 pb-6 space-y-5" style="scrollbar-width:thin;scrollbar-color:#d1d5db #f3f4f6;">
            ...
            <div class="bg-green-50 border border-green-200 rounded-xl px-4 py-3 flex items-start gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-green-700 mt-0.5 leading-snug">
            ...
            <div class="rounded-xl border border-gray-200 overflow-hidden bg-white shadow-sm">
                ...
                <div class="relative">
                    <img src="{{ $shareEventPhotoUrl }}"
                         alt="{{ $shareEventTitle }}"
                         class="w-full h-44 object-cover {{ $isCompleted ? 'brightness-90' : '' }}"
                         onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">
                    {{-- Fallback gradient if photo fails --}}
                    <div class="hidden w-full h-44 bg-gradient-to-br from-[#7a3f91] to-[#4c1d95] items-center justify-center">
                        <i class="fas fa-calendar-days text-white text-5xl opacity-60"></i>
                    </div>
                    {{-- Status badge over photo --}}
                    <div class="absolute top-2 right-2">
                        @if($isCompleted)
                            <span class="inline-flex items-center gap-1 px-2.5 py-1 bg-green-600 text-white rounded-full text-xs font-bold shadow">
                                <i class="fas fa-star text-[8px]"></i> Highlights
                            </span>
                        @else
                            <span class="inline-flex items-center gap-1 px-2.5 py-1 bg-emerald-600 text-white rounded-full text-xs font-bold shadow">
                ...
                <div class="px-4 py-4 bg-white">
                    <p class="font-bold text-gray-900 text-base leading-tight mb-2">
                        {{ $shareEventTitle }}
                    </p>
                    <div class="flex flex-wrap gap-2 mb-2">
                        <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded text-xs font-semibold bg-purple-50 text-purple-700 border border-purple-100">
                            <i class="fas fa-calendar text-[8px]"></i>{{ $shareEventDate }}
                        </span>
                        <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-100">
                            <i class="fas fa-clock text-[8px]"></i>{{ $timeDisplay }}
                        </span>
                        <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded text-xs font-semibold bg-gray-100 text-gray-700">
                            <i class="fas fa-location-dot text-[8px]"></i>{{ $shareEventVenue }}
                        </span>
                        @if($shareEventTarget)
                        <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded text-xs font-semibold bg-amber-50 text-amber-700 border border-amber-100">
                            <i class="fas fa-graduation-cap text-[8px]"></i>{{ $shareEventTarget }}
                        </span>
                        @endif
                    </div>
                    @if($previewDesc)
                    <p class="text-sm text-gray-500 leading-relaxed line-clamp-2">{{ $previewDesc }}</p>
                    @endif
                </div>

                {{-- OG card footer --}}
                <div class="px-4 py-2.5 bg-[#f0f2f5] flex items-center gap-2">
                    <i class="fas fa-globe text-gray-400 text-xs"></i>
                    <span class="text-xs text-gray-500 uppercase tracking-wide font-semibold">{{ strtoupper($shareHost) }}</span>
                </div>
            </div>
            {{-- ── End preview card ─────────────────────────────────────────── --}}
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 flex items-start gap-2.5">
                <i class="fas fa-circle-info text-blue-600 text-base flex-shrink-0 mt-0.5"></i>
            ...
            {{-- ── Share via label ───────────────────────────────────────────── --}}
            <p class="text-xs font-bold text-gray-500 uppercase tracking-widest pt-1">Share via</p>
            ...
            <button type="button"
                ...
                <span class="w-9 h-9 bg-white rounded-lg flex items-center justify-center flex-shrink-0 shadow-sm group-hover:scale-105 transition-transform">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5">
                        ...
                        <path fill="url(#mgr-ev)" d="M12 0C5.373 0 0 4.974 0 11.111c0 3.498 1.744 6.614 4.469 8.652V24l4.088-2.242c1.092.3 2.246.464 3.443.464 6.627 0 12-4.974 12-11.111S18.627 0 12 0zm1.191 14.963l-3.055-3.26-5.963 3.26 6.559-6.963 3.13 3.26 5.889-3.26-6.56 6.963z"/>
                    </svg>
                </span>
                <span class="flex-1 text-left">
            ...
            <button type="button"
                ...
                <div class="flex-1 text-left">
                    ...
                    <p class="text-xs text-gray-400 font-mono mt-1">{{ $shareBaseUrl }}</p>
                </div>
            </button>

            {{-- ── Footer note ─────────────────────────────────────────────── --}}
            <p class="text-xs text-gray-500 text-center leading-snug pb-1">
                @if($isCompleted)
                    <i class="fas fa-star text-green-500 mr-1"></i>Share this as an <strong class="text-gray-700">event recap</strong> para makita ng lahat ang naging tagumpay!
                @else
                    <i class="fas fa-circle-check text-emerald-500 mr-1"></i>This event is <strong class="text-gray-700">approved & live</strong> — safe to share with alumni!
                @endif
            </p>
        </div>
    </div>
