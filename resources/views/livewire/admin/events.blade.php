...
new class extends Component {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // AUTO-COMPLETE LOGIC (admin-wide, all organizers)
    // ─────────────────────────────────────────────────────────────────────────
...
<div class="px-4 sm:px-6 lg:px-8 py-6 max-w-screen-2xl mx-auto space-y-5">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-xl sm:text-2xl lg:text-3xl font-extrabold text-gray-900 tracking-tight">Event Management</h1>
                <p class="text-gray-500 text-xs sm:text-sm mt-0.5">Review, moderate, and manage all event postings.</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex flex-col"
        ...
        <div class="px-4 sm:px-6 py-3 border-b border-gray-200 bg-gray-50 flex flex-wrap gap-2 items-center">
            <div class="relative flex-1 min-w-[160px] max-w-xs"
                ...
                <input type="text" x-model="q" @input.debounce.400ms="$wire.set('search',q)"
                       ...
                       class="w-full pl-8 pr-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-900 bg-white focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-100 transition"
                       autocomplete="off">
            </div>

            <select wire:model.live="filterStatus"
                    class="px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white text-gray-700 focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-100 transition">
            ...
            <select wire:model.live="filterCollege"
                    class="px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white text-gray-700 focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-100 transition hidden sm:block">
        ...
        <div class="px-4 py-2 border-b border-gray-200 bg-gray-50 flex gap-2 sm:hidden">
            <select wire:model.live="filterCollege"
                    class="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white text-gray-700 focus:outline-none focus:border-purple-500 transition">
                <option value="">All Colleges</option>
                @foreach($this->colleges as $col)
                    <option value="{{ $col }}">{{ $col }}</option>
                @endforeach
                        ...
                        <tr class="
                            ...
                            <td class="px-4 sm:px-5 py-3.5 text-center whitespace-nowrap">
                                ...
                                @elseif($event->status === 'PENDING')
                                    <span class="inline-block px-2.5 py-1 bg-amber-50 text-amber-700 border border-amber-200 rounded-full text-[11px] font-bold">Pending</span>
                                @elseif($isApproved)
        ...
        <div class="px-4 sm:px-6 py-3.5 border-t border-gray-200 bg-[#2b0d3e] shrink-0">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                <p class="text-white text-xs sm:text-sm">Showing <span class="font-bold">{{ $from }}–{{ $to }}</span> of <span class="font-bold">{{ $total }}</span> events</p>
                <div class="flex items-center gap-1.5">
                    ...
                    @else
                        <button wire:click="previousPage" class="px-3 sm:px-4 py-2 bg-[#7a3f91] hover:bg-[#5e2f72] text-white rounded-lg text-xs sm:text-sm font-semibold transition">← Prev</button>
                    @endif
                    <span class="px-3 py-2 bg-white border border-gray-200 rounded-lg text-gray-700 text-xs sm:text-sm font-semibold shadow-sm">{{ $cp }} / {{ $this->events->lastPage() }}</span>
                    @if($this->events->hasMorePages())
                        <button wire:click="nextPage" class="px-3 sm:px-4 py-2 bg-[#7a3f91] hover:bg-[#5e2f72] text-white rounded-lg text-xs sm:text-sm font-semibold transition">Next →</button>
                    @else
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/60 backdrop-blur-sm"
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col overflow-hidden"
        ...
        <div class="flex items-center justify-between px-5 sm:px-7 py-4 bg-[#7a3f91] text-white flex-shrink-0">
            <h2 class="text-lg font-extrabold flex items-center gap-2.5"><i class="fas fa-pen-to-square"></i> Edit Event</h2>
        ...
        <div class="flex-1 min-h-0 overflow-y-auto px-5 sm:px-7 py-5 space-y-5" x-ref="formScroll" style="scrollbar-width:thin;scrollbar-color:#d1d5db #f9fafb;">
            ...
            <div>
                ...
                <div x-data="{isDragging:false}" @dragover.prevent="isDragging=true" @dragleave.prevent="isDragging=false" @drop.prevent="isDragging=false"
                     ...
                     :class="isDragging?'border-purple-400 bg-purple-50':'{{ ($photo||($existingPhotoUrl&&!$removePhoto))?'border-purple-300 border-solid bg-purple-50/40':'border-dashed border-gray-300 hover:border-purple-400 hover:bg-gray-50' }}'">
                    <label class="cursor-pointer block">
                        ...
                        @else
                            <div class="flex flex-col items-center gap-2 py-2"><i class="fas fa-cloud-arrow-up text-3xl text-gray-300"></i><p class="font-semibold text-gray-500 text-sm">Click to upload or drag & drop</p><p class="text-xs text-gray-400">JPG, PNG, WEBP — max 5 MB</p></div>
                        @endif
                    </label>
                </div>
                @if($existingPhotoUrl&&!$removePhoto&&!$photo)
                    <div class="mt-2 flex items-center gap-2">
                        <button type="button" wire:click="$set('removePhoto',true)" class="text-xs text-red-600 hover:text-red-700 font-semibold flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-red-200 hover:bg-red-50 transition"><i class="fas fa-trash text-[10px]"></i> Remove photo</button>
                ...
                @if($removePhoto)
                    <div class="mt-2 flex items-center gap-2">
                        ...
                        <button type="button" wire:click="$set('removePhoto',false)" class="text-xs text-blue-500 underline hover:text-blue-700">Undo</button>
                    </div>
                @endif
                ...
                <div class="p-4 sm:p-5 space-y-4">
                    <div>
                        ...
                        <input wire:model.defer="title" type="text" placeholder="e.g. PHILCST Alumni Homecoming 2026"
                               class="w-full px-4 py-2.5 border rounded-lg text-sm text-gray-900 bg-white transition focus:outline-none focus:ring-2 {{ isset($formErrors['title'])?'border-red-400 bg-red-50 focus:border-red-400 focus:ring-red-100':'border-gray-300 focus:border-purple-500 focus:ring-purple-100' }}">
                    ...
                    <div>
                        ...
                        <textarea wire:model.defer="description" rows="3" placeholder="Describe the event…"
                                  class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-900 bg-white focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-100 transition resize-none"></textarea>
                    </div>
                    <div>
                        ...
                        <input wire:model="event_date" type="date" min="{{ now('Asia/Manila')->format('Y-m-d') }}"
                               class="w-full px-4 py-2.5 border rounded-lg text-sm text-gray-900 bg-white transition focus:outline-none focus:ring-2 {{ isset($formErrors['event_date'])?'border-red-400 bg-red-50 focus:border-red-400 focus:ring-red-100':'border-gray-300 focus:border-purple-500 focus:ring-purple-100' }}">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input wire:model="start_time" type="text" placeholder="e.g. 8:00 AM"
                                   class="w-full px-4 py-2.5 border rounded-lg text-sm text-gray-900 bg-white transition focus:outline-none focus:ring-2 {{ isset($formErrors['start_time'])?'border-red-400 bg-red-50 focus:border-red-400 focus:ring-red-100':'border-gray-300 focus:border-purple-500 focus:ring-purple-100' }}">
                        ...
                        <div>
                            ...
                            <input wire:model="end_time" type="text" placeholder="e.g. 5:00 PM"
                                   class="w-full px-4 py-2.5 border rounded-lg text-sm text-gray-900 bg-white transition focus:outline-none focus:ring-2 {{ isset($formErrors['end_time'])?'border-red-400 bg-red-50 focus:border-red-400 focus:ring-red-100':'border-gray-300 focus:border-purple-500 focus:ring-purple-100' }}">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input wire:model.defer="venue" type="text" placeholder="e.g. PHILCST Main Gym"
                                   class="w-full px-4 py-2.5 border rounded-lg text-sm text-gray-900 bg-white transition focus:outline-none focus:ring-2 {{ isset($formErrors['venue'])?'border-red-400 bg-red-50 focus:border-red-400 focus:ring-red-100':'border-gray-300 focus:border-purple-500 focus:ring-purple-100' }}">
                        ...
                        <div>
                            ...
                            <input wire:model.defer="venue_address" type="text" placeholder="e.g. Old Nalsian Road, Calasiao, Pangasinan"
                                   class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-900 bg-white focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-100 transition">
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="p-4 sm:p-5 space-y-4">
                    <div class="flex gap-3">
                        <button type="button" wire:click="$set('targetMode','all')"
                                class="flex-1 py-3 px-3 border-2 rounded-xl text-xs font-bold transition flex flex-col items-center gap-1.5
                                       {{ $targetMode==='all' ? 'border-purple-500 bg-[#7a3f91] text-white' : 'border-gray-200 text-gray-500 hover:border-purple-300 hover:bg-purple-50 bg-white' }}">
                            <i class="fas fa-globe text-base"></i><span>All Colleges</span>
                        </button>
                        <button type="button" wire:click="$set('targetMode','college')"
                                class="flex-1 py-3 px-3 border-2 rounded-xl text-xs font-bold transition flex flex-col items-center gap-1.5
                                       {{ $targetMode==='college' ? 'border-purple-500 bg-[#7a3f91] text-white' : 'border-gray-200 text-gray-500 hover:border-purple-300 hover:bg-purple-50 bg-white' }}">
                                ...
                                <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                    @foreach($this->colleges as $col)
                                        <label class="flex items-center gap-2 px-3 py-2 border rounded-lg cursor-pointer transition text-xs font-semibold {{ in_array($col, $selectedColleges) ? 'border-purple-400 bg-purple-50 text-purple-700' : 'border-gray-200 text-gray-600 hover:border-purple-300 hover:bg-purple-50/40' }}">
                                            <input type="checkbox" wire:model.live="selectedColleges" value="{{ $col }}" class="accent-purple-600 w-3.5 h-3.5">
                                            <span>{{ $col }}</span>
                                        </label>
                                    @endforeach
                                </div>
                    ...
                    <div class="pt-3 border-t border-gray-100">
                        <label class="block text-xs font-bold text-gray-600 uppercase tracking-wide mb-1.5">Batch Year <span class="text-gray-400 font-normal normal-case">(Optional)</span></label>
                        <input wire:model.defer="batchYear" type="number" min="1990" max="{{ now()->year + 5 }}" placeholder="e.g. {{ now()->year - 2 }}"
                               class="w-full sm:max-w-xs px-4 py-2.5 border rounded-lg text-sm text-gray-900 bg-white transition focus:outline-none focus:ring-2 {{ isset($formErrors['batch_year'])?'border-red-400 bg-red-50 focus:border-red-400 focus:ring-red-100':'border-gray-300 focus:border-purple-500 focus:ring-purple-100' }}">
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden">
                ...
                <div class="p-4 sm:p-5 grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-600 uppercase tracking-wide mb-1.5">{{ $label }}</label>
                        <input wire:model.defer="{{ $field }}" type="{{ $type }}" placeholder="{{ $ph }}"
                               @if($editingIsOrganizerEvent) readonly @endif
                               class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-900 bg-white focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-100 transition {{ $editingIsOrganizerEvent ? 'cursor-not-allowed bg-gray-50 text-gray-500' : '' }}">
                    </div>
                    @endforeach
                    @if($editingIsOrganizerEvent)
                        <div class="col-span-1 sm:col-span-3"><p class="text-xs text-gray-400"><i class="fas fa-circle-info text-[10px] mr-1"></i>Contact details belong to the organizer and cannot be edited by admin.</p></div>
                    @endif
                </div>
            </div>
        ...
        <div class="px-5 sm:px-7 py-4 border-t border-gray-200 bg-gray-50 flex-shrink-0 flex gap-3">
            <button type="button" wire:click="closeFormModal" class="flex-1 px-4 py-2.5 border border-gray-300 text-gray-700 rounded-xl text-sm font-bold hover:bg-gray-100 transition">Cancel</button>
            <button type="button" wire:click="saveEvent" wire:loading.attr="disabled" wire:target="saveEvent"
                    class="flex-1 px-4 py-2.5 text-white rounded-xl text-sm font-extrabold bg-[#7a3f91] hover:bg-[#5e2f72] flex items-center justify-center gap-2 transition shadow-md disabled:opacity-50 disabled:cursor-not-allowed">
    ...
    <div class="bg-white rounded-2xl shadow-2xl flex flex-col w-full max-w-2xl overflow-hidden relative"
        ...
        <div class="px-5 sm:px-7 pt-5 pb-4 border-b border-gray-200 flex-shrink-0">
            ...
            <div class="flex items-start justify-between gap-3 mb-4">
                ...
                @elseif($isOrgDeleted)
                    <span class="flex-shrink-0 px-2.5 py-1 bg-red-100 text-red-700 border border-red-300 rounded-full text-[11px] font-bold">Deleted by Organizer</span>
            ...
            <ul class="space-y-2">
                ...
                @if($ev->target_participants)<li class="flex items-start gap-3 text-sm text-gray-700"><i class="fas fa-users" style="color:#7a3f91" class="mt-0.5 w-4 flex-shrink-0"></i><span>{{ $ev->target_participants }}</span></li>@endif
                <li class="flex items-start gap-3 text-sm text-gray-700"><i class="fas fa-{{ $ev->organizer ? 'user-tie' : 'shield-halved' }}" style="color:#7a3f91" class="mt-0.5 w-4 flex-shrink-0"></i><span>{{ $ev->organizer ? $ev->organizer->name . ' · ' . $ev->organizer->department : 'Posted by Admin' }}</span></li>
            </ul>
            ...
            <div class="px-5 sm:px-7 py-5 border-b border-gray-100">
                ...
                @if($isCompleted)
                    <div class="bg-green-50 border border-green-200 rounded-xl px-4 py-3">
                        ...
                        <p class="text-xs text-green-700 mt-1">This event has already taken place successfully.</p>
                    </div>
                @elseif($isOrgDeleted)
                    <div class="bg-red-50 border border-red-200 rounded-xl px-4 py-3"><p class="text-sm font-bold text-red-800"><i class="fas fa-trash mr-2 text-red-500"></i>Deleted by Organizer</p><p class="text-xs text-red-600 mt-1">You can restore this event to put it back to Pending review.</p></div>
                @elseif($ev->status==='PENDING')
                    <div class="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3"><p class="text-sm font-bold text-amber-800"><i class="fas fa-hourglass-half mr-2 text-amber-500"></i>Pending Admin Review</p><p class="text-xs text-amber-700 mt-1">This event is waiting for your approval.</p></div>
                @elseif($isApproved)
        ...
        <div class="px-5 sm:px-7 py-4 border-t border-gray-200 flex items-center justify-end gap-2 flex-wrap bg-white flex-shrink-0">
            ...
            @elseif($ev->status==='PENDING')
                ...
                <button wire:click="confirmApprove({{ $ev->id }})" class="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 hover:bg-emerald-100 rounded-xl transition"><i class="fas fa-check text-xs"></i> Approve</button>
            @elseif($ev->status==='REJECTED')
                <button wire:click="confirmDelete({{ $ev->id }})" class="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-bold text-red-600 border border-red-200 bg-white hover:bg-red-50 rounded-xl transition"><i class="fas fa-trash text-xs"></i> Delete</button>
                <button wire:click="confirmApprove({{ $ev->id }})" class="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 hover:bg-emerald-100 rounded-xl transition"><i class="fas fa-rotate-left text-xs"></i> Re-Approve</button>
            @endif
        </div>
    </div>
</div>
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" @keydown.escape.window="$wire.cancelApprove()">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100">
        <div class="px-6 py-5 bg-emerald-50 border-b border-emerald-100"><h2 class="text-base font-extrabold text-emerald-800 flex items-center gap-2.5"><div class="w-8 h-8 bg-emerald-100 rounded-xl flex items-center justify-center"><i class="fas fa-circle-check text-emerald-600 text-sm"></i></div>Approve Event</h2></div>
        <div class="p-6">
            ...
            <div class="mb-5">
                ...
                <textarea wire:model.defer="approveRemarks" rows="2" placeholder="e.g. Approved. Great event proposal!" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm text-gray-900 bg-white focus:outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 transition resize-none"></textarea>
            </div>
            <div class="flex gap-3">
                <button wire:click="cancelApprove" class="flex-1 px-4 py-2.5 border border-gray-300 text-gray-700 rounded-xl text-sm font-bold hover:bg-gray-50 transition">Cancel</button>
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" @keydown.escape.window="$wire.cancelReject()">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100">
        <div class="px-6 py-5 bg-red-50 border-b border-red-100"><h2 class="text-base font-extrabold text-red-800 flex items-center gap-2.5"><div class="w-8 h-8 bg-red-100 rounded-xl flex items-center justify-center"><i class="fas fa-circle-xmark text-red-500 text-sm"></i></div>Reject Event</h2></div>
        <div class="p-6">
            ...
            <div class="mb-5">
                ...
                <textarea wire:model.defer="rejectRemarks" rows="3" placeholder="e.g. Missing required details." class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm text-gray-900 bg-white focus:outline-none focus:border-red-400 focus:ring-2 focus:ring-red-100 transition resize-none"></textarea>
            ...
            <div class="flex gap-3">
                <button wire:click="cancelReject" class="flex-1 px-4 py-2.5 border border-gray-300 text-gray-700 rounded-xl text-sm font-bold hover:bg-gray-50 transition">Cancel</button>
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" @keydown.escape.window="$wire.cancelRestore()">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100">
        <div class="px-6 py-5 bg-orange-50 border-b border-orange-100"><h2 class="text-base font-extrabold text-orange-800 flex items-center gap-2.5"><div class="w-8 h-8 bg-orange-100 rounded-xl flex items-center justify-center"><i class="fas fa-rotate-left text-orange-500 text-sm"></i></div>Restore Event</h2></div>
        <div class="p-6">
            ...
            <div class="flex gap-3">
                <button wire:click="cancelRestore" class="flex-1 px-4 py-2.5 border border-gray-300 text-gray-700 rounded-xl text-sm font-bold hover:bg-gray-50 transition">Cancel</button>
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" @keydown.escape.window="$wire.cancelDelete()">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100">
        <div class="px-6 py-5 bg-red-50 border-b border-red-100"><h2 class="text-base font-extrabold text-red-800 flex items-center gap-2.5"><div class="w-8 h-8 bg-red-100 rounded-xl flex items-center justify-center"><i class="fas fa-triangle-exclamation text-red-500 text-sm"></i></div>Permanently Delete</h2></div>
        <div class="p-6">
            ...
            <div class="bg-red-50 border border-red-100 rounded-xl px-4 py-3 mb-5 text-xs text-gray-700 flex items-start gap-2"><i class="fas fa-exclamation-circle text-red-400 mt-0.5 shrink-0"></i><span>This action <strong>cannot be undone</strong>. The event and its photo will be permanently removed.</span></div>
            <div class="flex gap-3">
                <button wire:click="cancelDelete" class="flex-1 px-4 py-2.5 border border-gray-300 text-gray-700 rounded-xl text-sm font-bold hover:bg-gray-50 transition">Cancel</button>
